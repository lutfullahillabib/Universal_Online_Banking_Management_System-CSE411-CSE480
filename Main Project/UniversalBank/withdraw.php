                      ...
                      if(isset($_POST['withdraw']))
                      {
                        if($row["email"] == $_POST["email"] && $row["pass"] == md5($_POST["psw"]) && $row["pin"] == $_POST["pin"])
                        {
		                          ...
		                          $query = mysqli_query($conn,$sql);
															...
															$bank = $row['bank'];
															$amount = $_POST['amount'];
															$sql2 = "INSERT INTO transactions (name, acc, bank, amount, type) VALUES ('$fname','$account','$bank','$amount','withdraw');";
													    $query2 = $conn->query($sql2);
		                          echo "<p style='color: green; padding: 0px 10px;'>".$_POST['amount']." BDT has been successfully withdrawn from your account!</p>";
														}
														else {
